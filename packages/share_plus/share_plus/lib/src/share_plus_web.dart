...
class SharePlusWebPlugin extends SharePlatform {
  ...
  Future<ShareResult> shareWithResult(
    ...
    if (subject != null && subject.isNotEmpty) {
      data = ShareData.textWithSubject(
        title: subject,
        text: text,
      );
    } else {
    ...
    if (!_canShare(data)) {
      // share function is not available, try to handle mailto scheme via url_launcher
      await _handleMailToScheme(
        text: text,
        subject: subject,
      );
    ...
    try {
      ...
      // share action is not executed, try to handle mailto scheme via url_launcher
      await _handleMailToScheme(
        text: text,
        subject: subject,
      );
  ...
  Future<ShareResult> shareXFiles(
    ...
    if (text != null && text.isNotEmpty) {
      if (subject != null && subject.isNotEmpty) {
        data = ShareData.filesWithTextAndSubject(
          files: webFiles.toJS,
          text: text,
          title: subject,
        );
  ...
  Future<void> _handleMailToScheme({
...
extension type ShareData._(JSObject _) implements JSObject {
  ...
  external factory ShareData.textWithSubject({
    String text,
    String title,
  });
  ...
  external factory ShareData.filesWithTextAndSubject({
    JSArray<web.File> files,
    String text,
    String title,
  });
